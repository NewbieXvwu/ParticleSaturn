...
进展补充（2026-07-18）：Vulkan 场景图现叠加首个真实粒子通道：三条 32 字节共享 ABI 粒子记录上传为受控结构化存储缓冲，经 Diligent 缓冲视图绑定到运行时 GLSL 顶点着色器并以点图元绘制。MoltenVK 原生表面与主应用冒烟实际编译、绑定并呈现该通道；完整三缓冲模拟和动态粒子数量仍待迁入。
