...
inline void InitParticles(std::vector<GPUParticle>& particles) {
    struct TempParticle { float x, y, z, r, g, b, s, o, v, ring; };
    ...
    std::vector<glm::vec3> cols = { 
        HexToRGB(0xE3DAC5), HexToRGB(0xC9A070), 
        HexToRGB(0xE3DAC5), HexToRGB(0xB08D55) 
    };
    float R = 18.0f;
    ...
    for (unsigned int i = 0; i < MAX_PARTICLES; i++) {
        ...
        if (i < MAX_PARTICLES * 0.25) {
            ...
            if (ci < 0) ci = 0;
            p.r = cols[ci].x; p.g = cols[ci].y; p.b = cols[ci].z;
            p.s = 1 + rnd(gen) * 0.8f; p.o = 0.8f; p.v = 0; p.ring = 0;
        } else {
            // 土星环粒子（多层环带）
            float z = rnd(gen), rad;
            glm::vec3 c;
            if (z < 0.15) {
                ...
                c = HexToRGB(0x989085);
                p.s = 0.7f; p.o = 0.6f;
                if (rad > R * 2.2 && rad < R * 2.21) p.o = 0.1f;
            } else {
...
inline void CreateStars(unsigned int& vao, unsigned int& vbo, int count = 50000) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        starData.insert(starData.end(), {
            r * sin(ph) * cos(th), r * cos(ph), r * sin(ph) * sin(th),
            c.x, c.y, c.z,
            1 + rnd(gen) * 3
        });
    }
