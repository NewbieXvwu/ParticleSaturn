...
constexpr const char* UseObjectShaderKey = "render.useObjectShader";
...
App::AppState NSUserDefaultsStore::Load(const App::AppState& defaults) const {
    ...
    if (HasValue(values, UseObjectShaderKey)) state.render.useObjectShader = [values boolForKey:@"render.useObjectShader"];
...
void NSUserDefaultsStore::Save(const App::AppState& state) {
    ...
    [values setBool:state.render.useObjectShader forKey:@"render.useObjectShader"];
