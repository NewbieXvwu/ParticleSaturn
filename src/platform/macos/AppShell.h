...
// 帧高度接缝的类型规范定义移入 app/RenderSeam.h（D-015 重启，平台中立，
// macOS 与 Windows 外壳共用）。此处保留 Platform::MacOS 内的名字为别名，
// 现有 macOS 后端端点与外壳代码无需改动限定名。
using App::BackendCapabilities;
using App::BackendPanelHooks;
using App::FrameContext;
