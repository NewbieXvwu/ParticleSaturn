...
class OpenGLBloom {
  ...
  private:
    ...
    void DrawPass(unsigned int program, int sourceLocation, unsigned int sourceTexture, unsigned int targetFramebuffer,
                  std::uint32_t targetWidth, std::uint32_t targetHeight, std::uint32_t sourceWidth,
                  std::uint32_t sourceHeight, float offset, float threshold) const;
