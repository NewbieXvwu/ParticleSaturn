...
| 单源着色器（试点） | `src/shaders/single/ToneMap.hlsl` | tonemap 三路径已单源化（GL41 frag/Vulkan SPIR-V/Metal fragment）；构建前置 dxc（scripts/build_dxc_macos.sh）+ spirv-cross |
...
| macOS 三个模式 main | `src/platform/macos/{Main,OpenGL41Main,VulkanMain}.mm` | 已收口到 AppShell::RunApp：只剩后端构造 + 各自的 IRenderBackend 端点（Metal/OpenGL41/VulkanRenderBackend）+ ImGui 接线 |
