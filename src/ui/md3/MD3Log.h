#pragma once
// MD3 调试日志系统 —— 旧 DebugLog（src/DebugLog.h）的共享库版本。
//
// 完整保留旧版能力：
// - Info/Warn/Error 级别、过滤、搜索、复制
// - 重复日志合并、一次性日志（AddOnce）
// - ANSI 转义序列剥离、级别自动检测
// - 暂停/继续、自动滚动到底部
// - stdout/stderr 重定向捕获（MD3::DebugStreamBuf）
//
// 与旧版的差异仅在于依赖共享 MD3 库（"MD3.h"）并置于 MD3 命名空间，
// 避免与 Windows 目标各自的 md3 副本产生 ODR 冲突。
...
class DebugLog {
  ...
  private:
    ...
    static void StripAnsiEscapesInPlace(std::string& s) {
        // 删除 ANSI 转义序列（例如 "\x1b[39m"），避免 ImGui 日志出现 "[39m" 噪音。
        // 处理：
        // - CSI: ESC [ ... <final byte>
        // - OSC: ESC ] ... BEL
        // 同时移除丢失 ESC 字节的裸 "[39m"/"[0m" 片段。
        size_t w = 0;
