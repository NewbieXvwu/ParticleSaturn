...
7. **AUDIT 是冻结快照，本文件才是权威活状态（2026-07-29 血泪教训）**：`docs/AUDIT_2026-07.md` 记录的是 2026-07 某一时刻的债务，其后大量条目已在本 TODO 逐项清偿。**动手任何 AUDIT 条目前，必须先在本文件搜索对应项的完成勾选与备注、再落到磁盘核实当前实况**（文件是否还在、符号是否还被引用）。曾发生：直接照 AUDIT 快照建了 24 条 task，过半本机段其实早已完成，做了大量重复对账。建 task / 排工作前先读 TODO，不要只信 AUDIT。
8. **删除/退役任何文件前，跨"全部"构建系统核实无消费者（2026-07-29 血泪教训）**：本仓有多套构建入口——`CMakeLists.txt`（macOS + Windows Diligent）、每个 `*.vcxproj`（Windows MSBuild，如 `ParticleSaturn.OpenGL.vcxproj`）、`.github/workflows/release.yml`（CI job 如 build-opengl）。曾把 `src/OpenGL` 误判为死代码删除——因为只查了 CMake，漏了它是活跃的 MSBuild 发布目标（vcxproj + release.yml build-opengl job 均在，AUDIT 第 28/84 行已白纸黑字纠正过"src/OpenGL 是死代码"的误判）。**删除判定 = CMake ∧ 全部 vcxproj ∧ release.yml 三处都无引用**，缺一不可；grep 引用点时不要只搜 src/，要搜全仓。
...
- [x] `DiligentShaderSources.cpp` 死内联着色器切除（AUDIT P1-2 的一部分，2026-07-29）：全仓 grep（src+tests）核实 4 个 `Get*ShaderSources` 函数零调用者——`GetSaturnComputeShaderSource`、`GetStarShaderSources`、`GetSaturnParticleShaderSources`、`GetSaturnParticleMeshShaderSources`（macOS Vulkan adapter 走它自己的本地常量 `StarVertexShader/ParticleVertexShader/...`，Windows Diligent 已全面转字节码 `ShaderBytecodes.h` 不再调用任何 `Get*`）。连同仅被 mesh 死函数使用的 `MeshShaderSources` struct 一并删除 .cpp 定义与 .h 声明；文件 **2146→936 行（-1210）**。保留仍被 macOS Vulkan adapter 调用的 5 个函数（Fullscreen/BloomDownsample/BloomBlur/AcrylicComposite/SaturnInitCompute）。**本机验证**：`cmake --build build --target ParticleSaturnDiligentVulkanAdapter` 编译+链接全绿。**注**：5 个 live 函数的进一步去内联（转 .spv 字节码消费，部分已有 ToneMap.spv/BloomDownsample.spv/KawaseBlur.spv 回退路径）与 8 个 src/Diligent TU 的残留 `#include "DiligentShaderSources.h"` 噪音清理留待后续；Windows Diligent 目标同编译此文件（删的是零调用函数，安全），留 Windows 验证段
  - [x] Windows 验证段（2026-07-29 完成）：远程构建机 `C:\Users\NewbieXvwu\source\repos\ParticleSaturn` 经 `build_phaseB.bat`（vcvars64 + `cmake --build build --target ParticleSaturn.Diligent`）实机编译，`ninja: no work to do` + `BUILD_EXITCODE=0`，`ParticleSaturn.Diligent.exe` 已生成。**同批一并验证了 cc8e4a 巨物拆分**：7 个新 TU 的 obj 全部落盘（DiligentBloom/ParticleSystem/Present/RenderTargets/SevenSegment/StarField/UiComposite.cpp.obj），跨 TU include/ODR 无编译链接错误。注：远程构建机无法访问 github.com，代码经该机自身工作区（本批改动的原始来源）验证，非 pull 自 origin
- [x] P0-4 着色器创建样板去重（2026-07-29，提交 67c1caf）：新增 `PS_SHADER_FROM_BYTECODE` 宏（DiligentBackendInternal.h），把 6 个 TU（Bloom/RenderTargets/SevenSegment/StarField/UiComposite/ParticleSystem）中 8 处重复的 `backend_==Vulkan?SPIRV:DXBC` if/else（每处 6-12 行）折叠为单行；`.cpp` 里 `_SPIRV/_DXBC` 引用清零，净 -67 行。Mesh 的 DXIL 单路（D3D12 专用、无 SPIRV 变体）保留。**远程实测**：7 TU 重编译 + 链接，BUILD_EXITCODE=0。**P0-4 剩余（高风险大重构，未做）**：17 个 Create*PSO 抽 PipelineBuilder（差异交织，需逐个核对描述符等价）、68+ RefCntAutoPtr 成员按特性聚合、DirectComposition/VulkanD3D12Interop 转 ifdef；另"347 处 backend_ 运行时检查编译期消除"premise 存疑（Windows 单 exe 运行时选后端，backend_ 是真运行时值，动前需先证伪，疑似 FastRelease 同类过时判断）
