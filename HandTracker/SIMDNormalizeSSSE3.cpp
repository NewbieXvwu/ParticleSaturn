...
void StoreNormalized12(__m128i bytes, float* destination) {
    ...
    const __m128i low16  = _mm_unpacklo_epi8(bytes, zero);
    const __m128i high16 = _mm_unpackhi_epi8(bytes, zero);
    _mm_storeu_ps(destination, _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpacklo_epi16(low16, zero)), scale));
    _mm_storeu_ps(destination + 4, _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpackhi_epi16(low16, zero)), scale));
    _mm_storeu_ps(destination + 8, _mm_mul_ps(_mm_cvtepi32_ps(_mm_unpacklo_epi16(high16, zero)), scale));
}
...
void NormalizeRGBSsse3(const std::uint8_t* src, float* dst, std::size_t pixelCount) {
    std::size_t index = 0;
    for (; index + 4U <= pixelCount; index += 4U) {
        StoreNormalized12(Load12Bytes(src + index * 3U), dst + index * 3U);
    }
...
void FlipHorizontalAndNormalizeSsse3(const std::uint8_t* src, float* dst, int width, int height) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        int         x              = 0;
        for (; x + 4 <= width; x += 4) {
            const int offset = (width - x - 4) * 3;
            StoreNormalized12(_mm_shuffle_epi8(Load12Bytes(sourceRow + offset), reverseRgb), destinationRow + x * 3);
        }
        for (; x < width; ++x) {
...
void FlipHorizontalAndBGR2RGBSsse3(const std::uint8_t* src, std::uint8_t* dst, int width, int height) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (; x + 4 <= width; x += 4) {
            const int     offset = (width - x - 4) * 3;
            const __m128i bytes  = _mm_shuffle_epi8(Load12Bytes(sourceRow + offset), reverseBgrToRgb);
