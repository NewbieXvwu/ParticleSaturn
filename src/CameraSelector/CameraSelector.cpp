...
static bool IsSystemDarkMode() {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize", 0,
        ...
        if (RegQueryValueExW(hKey, L"AppsUseLightTheme", nullptr, nullptr, reinterpret_cast<BYTE*>(&value), &size) ==
            ERROR_SUCCESS) {
            RegCloseKey(hKey);
            return value == 0;
        }
...
static void Render(DialogState& state) {
    ...
    state.renderer.DrawText(L"选择摄像头", titleRect, state.theme.textPrimary, 24.0f, DWRITE_TEXT_ALIGNMENT_CENTER,
                            DWRITE_PARAGRAPH_ALIGNMENT_CENTER, DWRITE_FONT_WEIGHT_SEMI_BOLD);

    // 副标题（更“产品化”一点）
    D2D1_RECT_F subtitleRect =
        D2D1::RectF(0, 40, static_cast<float>(state.windowWidth), static_cast<float>(TITLE_HEIGHT));
    ...
    divider.a *= 0.65f;
    D2D1_RECT_F dividerRect =
        D2D1::RectF(static_cast<float>(CARD_SPACING), static_cast<float>(TITLE_HEIGHT - 4),
                    static_cast<float>(state.windowWidth - CARD_SPACING), static_cast<float>(TITLE_HEIGHT - 3));
    ...
    for (size_t i = 0; i < state.cameras.size(); i++) {
        ...
        if (idx < static_cast<int>(state.cardAnims.size())) {
            state.renderer.DrawAnimatedCard(cardRect, state.cardAnims[idx], isSelected, isHovered, state.theme,
                                            state.dt);
        } else {
            // Fallback
        ...
        D2D1_RECT_F previewRect = D2D1::RectF(previewX, previewY, previewX + PREVIEW_WIDTH, previewY + PREVIEW_HEIGHT);
        ...
        // 预览角标：摄像头序号
        {
            ...
            state.renderer.DrawText(chipText, chipRect, state.theme.textPrimary, 12.0f, DWRITE_TEXT_ALIGNMENT_CENTER,
                                    DWRITE_PARAGRAPH_ALIGNMENT_CENTER, DWRITE_FONT_WEIGHT_SEMI_BOLD);
        }
        ...
        D2D1_RECT_F nameRect =
            D2D1::RectF(cardRect.left + 40, cardRect.bottom - 36, cardRect.right - 12, cardRect.bottom - 8);
    ...
    state.renderer.DrawAnimatedButton(cancelRect, L"取消", state.cancelButtonAnim, state.cancelHover,
                                      state.cancelPressed, state.theme, state.dt, false);

    D2D1_RECT_F okRect = GetOkButtonRect(state);
    state.renderer.DrawAnimatedButton(okRect, L"确定", state.okButtonAnim, state.okHover, state.okPressed, state.theme,
                                      state.dt, true);

    // 复选框（带动画）
    D2D1_RECT_F checkboxRect = GetCheckboxRect(state);
    state.renderer.DrawAnimatedCheckbox(checkboxRect, state.checkboxAnim, state.rememberChoice, state.checkboxHover,
                                        state.checkboxPressed, state.theme, state.dt);

    // 复选框文字
    D2D1_RECT_F checkboxTextRect =
        D2D1::RectF(checkboxRect.right + 8, checkboxRect.top - 2, checkboxRect.right + 200, checkboxRect.bottom + 2);
...
static void SaveCameraChoice(int index) {
    HKEY hKey;
    if (RegCreateKeyExW(HKEY_CURRENT_USER, REGISTRY_KEY, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey,
                        nullptr) == ERROR_SUCCESS) {
        DWORD value = static_cast<DWORD>(index);
        RegSetValueExW(hKey, REGISTRY_VALUE, 0, REG_DWORD, reinterpret_cast<const BYTE*>(&value), sizeof(value));
        RegCloseKey(hKey);
    }
}
...
static void SaveSkipCameraSelectorDialog(bool skip) {
    ...
    if (RegCreateKeyExW(HKEY_CURRENT_USER, REGISTRY_KEY, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_WRITE, nullptr, &hKey,
        ...
        RegSetValueExW(hKey, REGISTRY_VALUE_SKIP, 0, REG_DWORD, reinterpret_cast<const BYTE*>(&value), sizeof(value));
        RegCloseKey(hKey);
    }
}
...
static bool GetSkipCameraSelectorDialog() {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, REGISTRY_KEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        DWORD size  = sizeof(value);
        DWORD type  = 0;
        if (RegQueryValueExW(hKey, REGISTRY_VALUE_SKIP, nullptr, &type, reinterpret_cast<BYTE*>(&value), &size) ==
                ERROR_SUCCESS &&
            type == REG_DWORD) {
            RegCloseKey(hKey);
            return value != 0;
...
int GetSavedCameraChoice() {
    ...
    if (RegOpenKeyExW(HKEY_CURRENT_USER, REGISTRY_KEY, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        ...
        DWORD size  = sizeof(value);
        DWORD type  = 0;
        if (RegQueryValueExW(hKey, REGISTRY_VALUE, nullptr, &type, reinterpret_cast<BYTE*>(&value), &size) ==
                ERROR_SUCCESS &&
            type == REG_DWORD) {
            RegCloseKey(hKey);
...
int ShowCameraSelectorDialog(HWND parentHwnd, HINSTANCE hInstance, bool forceShow) {
    ...
    state.hwnd = CreateWindowExW(0, WINDOW_CLASS, L"选择摄像头",
