...
public:
    OneEuroFilter(float minCutoff = 1.0f, float beta = 0.007f, float dCutoff = 1.0f)
        : minCutoff(minCutoff), beta(beta), dCutoff(dCutoff),
          firstTime(true), prevValue(0), prevDx(0) {}
...
void WorkerThreadFunc(int cam_id, std::string model_dir) {
    ...
    if (g_palm_model_data && g_palm_model_size > 0) {
        ...
        std::string palm_path = JoinPath(model_dir, "palm_detection_nchw.onnx");
        std::string landmark_path = JoinPath(model_dir, "hand_landmark_nchw.onnx");
    ...
    try { cap.open(cam_id, cv::CAP_DSHOW); } catch (...) {}
    if (cap.isOpened()) {
        camera_ok = true;
    } else {
        try { cap.open(cam_id, cv::CAP_MSMF); } catch (...) {}
        if (cap.isOpened()) {
            camera_ok = true;
        } else {
            try { cap.open(cam_id, cv::CAP_ANY); } catch (...) {}
            if (cap.isOpened()) camera_ok = true;
        }
    }
    ...
    while (g_running) {
        ...
        if (!palms.empty()) {
            ...
            // 检测手部关键点
            std::vector<cv::Point2f> landmarks;
            float presence = landmark_detector.detect(roi_image, trans_mat_inv, landmarks);
            
            bool landmarks_valid = (landmarks.size() >= 21 && presence > 0.3f);
        ...
        if (g_debug_mode) {
            ...
            if (current_has_hand && !palms.empty()) {
                ...
                cv::rectangle(debug_frame, 
                    cv::Point((int)(rect.x * img_w), (int)(rect.y * img_h)),
                ...
                for (int i = 0; i < 4; i++) {
                    cv::line(debug_frame,
                        ...
                        cv::Point((int)(palm.hand_pos[(i+1)%4].x * img_w), (int)(palm.hand_pos[(i+1)%4].y * img_h)),
                ...
                if (debug_landmarks_valid && debug_landmarks.size() >= 21) {
                    ...
                    for (auto& conn : connections) {
                        cv::line(debug_frame,
                            cv::Point((int)lm[conn[0]].x, (int)lm[conn[0]].y),
                            cv::Point((int)lm[conn[1]].x, (int)lm[conn[1]].y),
                            cv::Scalar(0, 255, 0), 2);
                    }
                    ...
                    for (int i = 0; i < 21; i++) {
                        ...
                        else if (i == 4) color = cv::Scalar(0, 0, 255);
                        else if (i == 8) color = cv::Scalar(0, 0, 255);
                        else color = cv::Scalar(0, 255, 255);
                    ...
                    cv::line(debug_frame,
                        cv::Point((int)lm[4].x, (int)lm[4].y),
                        cv::Point((int)lm[8].x, (int)lm[8].y),
                        cv::Scalar(0, 0, 255), 3);
                }
            }
            
            char info[256];
            snprintf(info, sizeof(info), "Scale: %.2f  RotX: %.2f  RotY: %.2f", 
                g_smooth_scale, g_smooth_rot_x, g_smooth_rot_y);
            cv::putText(debug_frame, info, cv::Point(10, 30), 
                cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0), 2);
            
            char hand_info[128];
            snprintf(hand_info, sizeof(hand_info), "Hand: %s (raw: %s, lost: %d)", 
                smooth_has_hand ? "YES" : "NO",
                current_has_hand ? "Y" : "N",
                hand_lost_counter);
            cv::putText(debug_frame, hand_info, cv::Point(10, 60), 
...
HAND_API bool InitTracker(int camera_id, const char* model_dir) {
    ...
    g_running = true;
    try {
        g_worker_thread = new std::thread(WorkerThreadFunc, camera_id, 
            model_dir ? std::string(model_dir) : std::string());
    } catch (...) {
        g_running = false;
        if (g_worker_thread) {
...
HAND_API bool GetHandData(float* out_scale, float* out_rot_x, float* out_rot_y, bool* out_has_hand) {
    ...
    if (out_scale) *out_scale = g_latest_data.scale;
    if (out_rot_x) *out_rot_x = g_latest_data.rot_x;
    if (out_rot_y) *out_rot_y = g_latest_data.rot_y;
    if (out_has_hand) *out_has_hand = g_latest_data.has_hand;
