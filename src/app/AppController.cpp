...
CommandEffect AppController::Dispatch(const AppCommand& command) {
    return std::visit(
        [this](const auto& value) -> CommandEffect {
            ...
            if constexpr (std::is_same_v<Command, SetParticleCount>) {
            ...
            } else if constexpr (std::is_same_v<Command, SetUseObjectShader>) {
                effect.renderSettingsChanged  = state_.render.useObjectShader != value.value;
                state_.render.useObjectShader = value.value;
                ...
                switch (value.key) {
                case InputKey::F3:
                    pressed = &state_.input.keyF3Pressed;
                    break;
                case InputKey::F11:
                    pressed = &state_.input.keyF11Pressed;
                    break;
                case InputKey::B:
                    pressed = &state_.input.keyBPressed;
                    break;
                case InputKey::Escape:
                    pressed = &state_.input.keyEscapePressed;
                    break;
                }
