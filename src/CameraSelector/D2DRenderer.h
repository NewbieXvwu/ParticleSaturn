...
struct CardAnimState {
    ...
    CardAnimState()
        : hoverState(0.0f, 400.0f, 28.0f),
          selectState(0.0f, 350.0f, 25.0f),
          elevation(0.0f, 300.0f, 25.0f) {}
};
...
public:
    ...
    // 绘制带动画的复选框
    void DrawAnimatedCheckbox(const D2D1_RECT_F& rect,
                               CheckboxAnimState& state,
                               bool checked, bool hovered, bool pressed,
                               const Theme& theme, float dt);

    // 绘制带动画的按钮
    void DrawAnimatedButton(const D2D1_RECT_F& rect,
    ...
    // 绘制带动画的卡片
    void DrawAnimatedCard(const D2D1_RECT_F& rect,
                          CardAnimState& state,
                          bool selected, bool hovered,
                          const Theme& theme, float dt);

    // Ripple 效果
    void TriggerRipple(float centerX, float centerY,
