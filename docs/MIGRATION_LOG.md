...
进展补充（2026-07-26，对象着色器基准）：以启用断言的独立编译真实运行像素基准（Release 构建的 `assert` 因 `NDEBUG` 失效，此前注册测试为空跑），暴露并修复了四个问题：基准测试自身的参数错误（未接收 `metallib` 路径、星体初始化缺参、渲染目标格式与生产不符）；对象着色器管线颜色附件为 BGRA8 且混合与传统粒子管线不一致（生产 `scene-hdr` 通道为 RGBA16Float，开启对象着色器将因格式不匹配无法绘制），已改为 RGBA16Float 与 SrcAlpha/One 加色混合；object 阶段 32 线程竞争写共享 payload 且每组只派生一个 mesh 线程组（仅绘制 1/32 粒子），已重写为单线程 object 组记录段基址、按活动粒子数派生至多 32 个 mesh 组、mesh 组按 `threadgroup_position_in_grid` 取粒子，网格面片的横向半宽改为除以宽高比以保持与点精灵一致的像素覆盖；宿主端补充 `setMeshBuffer`/`setMeshBytes`（mesh 阶段绑定表独立，此前读取未绑定缓冲）。修复后 1280×720、20 万粒子固定状态下两路径平均通道差 0.34（阈值 2.0）、差异像素 1.2%（阈值 4%），基准在本机 Apple M5 Pro 实测通过。
...
### 2026-07-27 四后端对比矩阵补齐：KosmicKrisp 首组数据 + 双 ICD 收敛结论（TODO P4，D-001/D-013）

对比模式首次跑满四后端（metal / opengl41 / vulkan-molten / vulkan-kosmic）。KosmicKrisp 对 Metal 的逐 pass 距离与 MoltenVK 完全同噪（scene-hdr 1.0147 vs 1.0141、失配同为 0.0004%；bloom 0.0460 vs 0.0459、失配均 0）。Molten 与 Kosmic **直接对比**：总帧 0.010/0.005%、scene-hdr 0.007/0%、bloom 0.001/0%——同一 SPIR-V 经两家独立 ICD 近逐位一致。**结论**：对 Metal 的 ~1 LSB 场景残差是 Vulkan 翻译路径的共性（着色器经 SPIR-V 翻译与 API 语义映射），不是任一驱动的个性；D-013 双 ICD 并列的实验价值首次有了量化底数。GL41 距离与三方一致（1.741/3.17%，全部落在已定案的场景 pass 分歧内）。
...
### 2026-07-26 ctest LABELS 分层 + macOS CI（TODO P0 第 4 项，AUDIT P2-9/P0-2(d)）
